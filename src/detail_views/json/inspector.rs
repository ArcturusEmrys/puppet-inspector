...
use crate::detail_views::json::pathing::{JsonPath, JsonValueExt};
use crate::detail_views::json::subkey::Subkey;
...
impl JsonInspector {
    ...
    fn bind(&self) {
        ...
        if let Some(value) = value.and_then(|v: &JsonValue| v.traverse_path(path)) {
            ...
            match value {
                JsonValue::Object(obj) => {
                    for (key, _) in obj.iter() {
                        keys.push(Subkey::new_object_key(key.to_string()));
                    }
                }
                JsonValue::Array(list) => {
                    for (index, _) in list.iter().enumerate() {
                        keys.push(Subkey::new_list_index(index));
                    }
                }
                _ => {}
            }
            ...
            self.imp().key_factory.connect_setup(|factory, object| {
                let list_item = object.downcast_ref::<gtk4::ListItem>().unwrap();
                list_item.set_child(Some(&gtk4::Label::builder().build()));
            });

            self.imp().key_factory.connect_bind(|factory, object| {
                let list_item = object.downcast_ref::<gtk4::ListItem>().unwrap();
                let subkey_item = list_item.item().unwrap();
                let subkey = subkey_item.downcast_ref::<Subkey>().unwrap();
                ...
                label.set_label(&match subkey.as_jsonnavpath() {
                    JsonNavigationPath::ObjectKey(key) => key.escape_nulls().into_owned(),
            ...
            self.imp().type_factory.connect_setup(|factory, object| {
                let list_item = object.downcast_ref::<gtk4::ListItem>().unwrap();
                list_item.set_child(Some(&gtk4::Label::builder().build()));
            });
            ...
            self.imp()
                .type_factory
                .connect_bind(move |factory, object| {
                    let list_item = object.downcast_ref::<gtk4::ListItem>().unwrap();
                    let subkey_item = list_item.item().unwrap();
                    let subkey = subkey_item.downcast_ref::<Subkey>().unwrap();
                    let label_child = list_item.child().unwrap();
                    ...
                    let value: Option<&JsonValue> =
                        value.and_then(|value| value.traverse_path(path.as_slice()));
            ...
            self.imp().value_factory.connect_setup(|factory, object| {
                let list_item = object.downcast_ref::<gtk4::ListItem>().unwrap();
                list_item.set_child(Some(&gtk4::Label::builder().build()));
            });
